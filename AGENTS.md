...
- CI/CD 구성과 정적 검사는 서비스 개발 전에 준비할 수 있다. 서비스의 실행 가능한 골격·테스트·빌드 명령이 생기는 변경부터 해당 검사를 연결하며, 전체 기능 완성을 기다리지 않는다.
- 미구현·비활성·건너뛴 작업을 테스트/배포 성공으로 보고하지 않는다. 실제 배포 실행 조건과 CI 단계별 기준은 [검증 기준](docs/verification.md)을 따른다.
